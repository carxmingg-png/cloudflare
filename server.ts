...
export const CATALOG_86_CARS: string[] = [
...
export const TOTAL_PREMIUM_CARS: number = CATALOG_86_CARS.length;
...
function ensureCarToRealEstateSlot(profile: any) {
...
function assignCarToFreeSlot(profile: any, carId: string, templateSlotsObj?: any): string {
  ...
  const existingIdx = profile.car_to_real_estate_slot.keys.findIndex((k: any) => k.toString() === carStr);
  if (existingIdx !== -1) {
    const slot = profile.car_to_real_estate_slot.values[existingIdx];
    if (slot && profile.real_estate_slots[slot]) {
      const houseName = slot.split("_slot_")[0];
      const isHouseBought = houseName === "apartment_95" ||
      ...
      if (isHouseBought) {
        ...
        delete profile.real_estate_slots[slot].car_id;
        if (slot !== "apartment_95_slot_0") {
          delete profile.real_estate_slots[slot].unlocked;
        }
      }
    }
  }
  ...
  if (slots.length === 0 && templateSlotsObj) {
    profile.real_estate_slots = JSON.parse(JSON.stringify(templateSlotsObj));
    slots = Object.keys(profile.real_estate_slots);
  }

  const slotsInUse = new Set<string>();
  ...
  const sortedSlots = slots.sort((a, b) => {
    const aIsApt = a.startsWith("apartment_95") ? 0 : 1;
    const bIsApt = b.startsWith("apartment_95") ? 0 : 1;
    return aIsApt - bIsApt;
  });
  ...
  for (const slot of sortedSlots) {
    ...
    if (!isHouseBought) continue;

    const isSlotFree = !slotsInUse.has(slot) && !profile.real_estate_slots[slot]?.car_id;
    if (isSlotFree) {
      targetSlot = slot;
      break;
    }
  ...
  const kIdx = profile.car_to_real_estate_slot.keys.findIndex((k: any) => k.toString() === carStr);
  if (kIdx !== -1) {
...
function sanitizeCarToRealEstateSlot(profile: any) {
  ensureCarToRealEstateSlot(profile);
  const keys = profile.car_to_real_estate_slot.keys || [];
  const values = profile.car_to_real_estate_slot.values || [];
  ...
  for (let i = 0; i < keys.length; i++) {
    const carId = keys[i]?.toString();
    const slot = values[i];
    
    const houseName = slot ? slot.split("_slot_")[0] : "";
    const isHouseBought = houseName === "apartment_95" ||
      (profile.real_estates[houseName] && profile.real_estates[houseName].is_bought === true);

    if (slot && carId && ownedCarIds.has(carId) && isHouseBought && !usedSlots.has(slot)) {
      usedSlots.add(slot);
      newKeys.push(keys[i]);
      newValues.push(slot);
    }
  ...
  if (profile.real_estate_slots) {
    for (const slotName in profile.real_estate_slots) {
      ...
      if (slotData && slotData.car_id !== undefined && slotData.car_id !== null) {
        const cidStr = slotData.car_id.toString();
        const houseName = slotName.split("_slot_")[0];
        const isHouseBought = houseName === "apartment_95" ||
          (profile.real_estates[houseName] && profile.real_estates[houseName].is_bought === true);
        
        if (!ownedCarIds.has(cidStr) || !isHouseBought) {
          delete profile.real_estate_slots[slotName].car_id;
          if (!isHouseBought && slotName !== "apartment_95_slot_0") {
            delete profile.real_estate_slots[slotName].unlocked;
          }
        }
      }
    }
...
export function modifyProfile(base: any, mods: {
  ...
  if (mods.get_all_cars) {
    ...
    for (const carId of carIds) {
      assignCarToFreeSlot(profile, carId, PROFILE_TEMPLATE?.real_estate_slots);
    }

    const currentCarExists = profile.current_car_id && profile.cars?.items?.[profile.current_car_id.toString()];
  ...
  if (mods.inject_car) {
    ...
    if (PREMIUM_BUILDS && PREMIUM_BUILDS[baseCarId]) {
  ...
  if (mods.inject_cars && Array.isArray(mods.inject_cars)) {
    ...
    for (const carId of mods.inject_cars) {
      const baseCarId = carId.replace(/_sp[12]/g, "");
      let carTemplateObj: any = null;
  ...
  // Sync car_models with garage items (dealership crash prevention)
